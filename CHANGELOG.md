...
### 新增
- `/run` 并发限制：同一用户同时只能运行 1 个任务
- `/run` 输入限制：代码长度、stdin 长度、单次超时上限
- WebSocket 登录 Token 首消息鉴权与超时
- 安全鉴权测试：`tests/test_auth_security.py`

### 变更
- `/run`、`/self-test` 改为 FastAPI 线程池执行
- WebSocket Token 从 URL Query 移到首条 `join` 消息
- 环境安装按钮仅管理员可见
- AI 历史、运行历史按用户隔离
- 清理 `.new` 冗余文件与重复 Logo

### 修复
- `/run`、`/self-test`、`/install`、AI 历史未鉴权可访问
- 代码执行阻塞 `/health`，实测健康检查超时
- 协作房间运行代码功能未进入真正运行的 `App.tsx`
- 课程数量恒为 0、空用户名注册 500
- WebSocket Token 暴露在 URL 中
- 下载代码 Blob URL 未释放
...
### 改进
- **Monaco 编辑器自定义浅色主题**：浅灰背景 `#f6f8fa`，蓝色光标 `#044289`
  - 行号颜色 `#9da5b0`，当前行号 `#24292e`
  - 选区浅蓝高亮 `#c8ddf0`，括号匹配、补全弹窗全部适配浅色
  - 语法高亮：关键字 `#d73a49`、字符串 `#032f62`、数字 `#005cc5`、函数 `#6f42c1`
